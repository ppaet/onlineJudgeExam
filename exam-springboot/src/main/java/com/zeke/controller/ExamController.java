...
//        读取map数据注入exam
     ...
     * @return
     */
    @GetMapping("/selectOne/student/answer/{seId}")
    public ApiResult<ExamAnswer> selectAnswerStu(@PathVariable("seId") String seId) {
        return examService.selectAnswer(Integer.parseInt(seId));
    }
    ...
    public ApiResult<List<StudentExam>> submitList(@PathVariable("uId") Integer uId) {
        return examService.submitList(uId);
    }

    @GetMapping("/review/{uId}")
    public ApiResult<List<JSONObject>> examReview(@PathVariable("uId") Integer uId) {
        return examService.examReview(uId);
    }
    @GetMapping("/review/score")
    public ApiResult<Object> reviseScore(Integer seId, Double score) {
        return examService.reviseScore(seId, score);
    }

    @GetMapping("/review/comments")
    public ApiResult<Object> addComments(Integer seId, String comments) {
        return examService.addComments(seId, comments);
    }
}
