              ...
              >
                ...
                <div class="choice">
                  <div class="check-box">
                    ...
                    <!--填空或者编程题-->
                    <el-input
                        v-if="[4].includes(topic.answer.typeId)"
                        v-model="topic.answer.answerContent"
                        :rows="5"
                        type="textarea"
                        placeholder="请输入答案"
                    />
                    <el-input
                        v-if="[5].includes(topic.answer.typeId)"
                        v-model="topic.answer.answerContent"
                        :rows="5"
                        type="textarea"
                        placeholder="请输入答案"
                    ...
                    <el-button 
                      v-if="[5].includes(topic.answer.typeId)"
                      type="primary"
                      @click="testCode(topic.tId, topic.answer.answerContent)">
                      发起测试
                    </el-button>
                  </div>
                </div>
...
const changeStatus = computed(() => (tId) => {
  ...
  return topic.answer.answerContent.toString() != '';
})
...
const testCode = async (ttid, answerContent) => {
  const topicTest = {
    tId: ttid,
    code: answerContent,
  };
  const res = await request.post<any, ApiResult>('/oj/topicTest', topicTest)
  const isSuccess = res.msg == "SUCCESS";
  if (isSuccess) {
    MyElNotification(res, Code.SAVA_OK, res.msg)
  } else {
    ElNotification({
      title: '提示信息',
      message: '测试不通过',
