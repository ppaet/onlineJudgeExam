          ...
          <div class="content-wthree" style="border-radius: 0 8px 8px 0;">
            ...
            <el-form ref="ruleFormRef"
              ...
              <el-radio-group
                ...
                <el-radio
                    v-for="item in roleData.filter(item => item.rId !== 0)" :key="item.rId" :label="item.rId"
                    size="large"
                    style="margin: 0"
                >
                  {{ item.name }}
...
const sendEmail = () => {
  ruleFormRef.value?.validateField(['checkCode', 'email'], (isValid, invalidFields) => {
    if (isValid && sendButtonStatus.value) {
      request
          .post<any, ApiResult>('/auth/send_email', {'toEmail': ruleForm.email, 'type': "register"})
          .then((res) => {
